...
void drawMenu()
{
	...
	*/
	...
	switch (selection)
	{
	case 1: is_clear = true; display(); to_create_file();/*新建文件函数*/ drawMenu(); break; // is_clear = false; break;
	case 2: is_clear = true; display(); to_open_file();/*打开文件函数*/  drawMenu(); break; //is_clear = false; break;
	case 3:	is_clear = true; display(); to_close_file(); /*关闭文件函数*/  drawMenu(); break; //is_clear = false; break;
	case 4: is_clear = true; display(); to_save_file();/*保存文件函数*/ drawMenu(); break; //is_clear = false; break;
	case 5: is_clear = true; display(); to_save_file_as();/*另存为文件函数*/ drawMenu(); break; //is_clear = false; break;
	case 6: exit(-1);  break;
	}
	/*画编辑菜单*/
	...
	switch (selection)
	{
	case 1: is_clear = true; display(); to_display_as_line();/*折线显示函数*/  drawMenu(); break; //is_clear = false;  break;
	case 2: is_clear = true; display(); to_display_as_dot();/*点阵显示函数*/  drawMenu(); break; //is_clear = false;  break;
	case 3: is_clear = true; display(); to_display_as_bar();/*柱形显示函数*/ drawMenu(); break; //is_clear = false;  break;
	case 4: is_clear = true; display(); to_change_row_name();/*行名修改函数*/  drawMenu(); break; //is_clear = false;  break;
	case 5: is_clear = true; display(); to_change_col_name();/*列名修改函数*/ drawMenu(); break; //is_clear = false;  break;
	case 6: is_clear = true; display(); to_change_data_name();/*数据名修改函数*/  drawMenu(); break; //is_clear = false;  break;
	}
	/*画绘图菜单*/
	selection = menuList(GenUIID(0), x + 2 * w, y - h, w, wlist, h, menuListDraw, sizeof(menuListDraw) / sizeof(menuListDraw[0]));
	switch (selection)
	{
	case 1: is_clear = true; display(); to_show_dot();/*绘制点图函数*/  drawMenu(); break; //is_clear = false; break;
	case 2: is_clear = true; display(); to_show_line();/*绘制折线图函数*/  drawMenu(); break; //is_clear = false;  break;
	case 3: is_clear = true; display(); to_show_bar();/*绘制柱状图函数*/ drawMenu(); break; //is_clear = false;  break;
	case 4: is_clear = true; display(); to_show_pie();/*绘制饼状图函数*/  drawMenu(); break; //is_clear = false;  break;
	}
	/*画帮助菜单*/
	selection = menuList(GenUIID(0), x + 3 * w, y - h, w, wlist, h, menuListHelp, sizeof(menuListHelp) / sizeof(menuListHelp[0]));
	switch (selection)
	{
	case 1: is_clear = true; display(); flag = true; draw_statement(); /*使用说明函数*/ break; //is_clear = false;  break;
	case 2: is_clear = true; display(); flag2 = true;  draw_about(); /*关于函数*/ break; //is_clear = false;  break;
	}
...
void drawText()
{
	...
	*/
	...
	drawBox(4.0, 2.2, 11.3, 0.3, 1, s, 'L', "Scarlet");
	SetPenColor(color);
	if (Mytable) {
...
void drawGraphics()
{
	...
	drawRectangle(4.0, 2.5, 11.3, 6.0, 1);
}
//#endif
